...
module tb_cnn_top;
    ...
    // Debug output wires — must be declared before DUT instantiation
    ...
    // =========================================================================
    // GLS: Pre-initialize SRAM macro arrays to zero after reset
    // The real IHP macro starts with X; this matches behavioral model behaviour.
    // =========================================================================
    initial begin
        #1;
        ...
        $display("[TB] GLS: SRAM arrays initialized to zero at t=%0t ns", $time);
    end
    ...
    // =========================================================================
    always @(posedge clk) begin
        if (rst_n && ~u_dut.conv_out_valid && ^debug_conv_out_data === 1'bx)
    ...
    initial begin
        ...
        // GLS: check_bit("POST_RESET: mac_valid=0",      tb_mac_valid, 1'b0);
        // GLS: check_bit("POST_RESET: conv_out_valid=0", tb_conv_out_v,1'b0);
        // GLS: check_bit("POST_RESET: layer_done=0",     tb_layer_done,1'b0);

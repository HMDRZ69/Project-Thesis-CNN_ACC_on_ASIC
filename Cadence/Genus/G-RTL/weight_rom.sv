...
//
//   Formula (addr_gen must apply):
//     Conv1 (layer_sel=0): index =       (oc * 1 * 9) + (ic * 9) + k   →   0..35
//     Conv2 (layer_sel=1): index = 36 +  (oc * 4 * 9) + (ic * 9) + k   →  36..323
//
//   Equivalently: index = layer_offset + (oc * Cin + ic) * 9 + k
//     where layer_offset = 0 for Conv1, 36 (= 4*1*9) for Conv2.
//
...
module weight_rom #(
    ...
    input  wire logic [ADDR_W-1:0]      index,

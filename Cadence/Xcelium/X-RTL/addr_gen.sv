...
//   [x] 4-lane activation SRAM reads
// =============================================================================
...
module addr_gen #(
    ...
    input  wire logic   enable,      // 1 => run address generation
    input  wire logic   layer_sel,   // 0: Conv1 (Cin=1,Cout=4)  1: Conv2 (Cin=4,Cout=8)
    ...
    // tap_grp counts 4-tap groups (0..8 for Conv2, 0..2 for Conv1).
    // The actual first-tap index is tap_grp*4, so bits [1:0] of the old
    // 6-bit tap_base were structurally always zero. Using a 4-bit group
    // counter eliminates those dead register bits (Genus GLO-45 warning).
    logic [3:0] tap_grp;   // tap group index; tap_base = {tap_grp, 2'b00}
    ...
    // Equivalent to: (tap_grp*4 + 4) >= taps_total
    logic last_tap_group;
    always_comb begin
        last_tap_group = (({tap_grp, 2'b00} + 6'd4) >= taps_total);
    end
            ...
            begin : gen_lanes_block
                ...
                for (int lane = 0; lane < 4; lane++) begin
                    ...
                    if (tap_i < int'(taps_total)) begin
                        w_rd_en[lane] = 1'b1;
                        // Weight ROM layout:
                        //   Conv1 (layer_sel=0): indices   0..35  → offset = 0
                        //   Conv2 (layer_sel=1): indices  36..323 → offset = 36
                        // offset = Cout1 * Cin1 * 9 = 4 * 1 * 9 = 36
                        w_idx[lane] = 9'((int'(oc) * int'(cin_p) + ic_i) * 9
                                         + k_i
                                         + (layer_sel ? 36 : 0));
